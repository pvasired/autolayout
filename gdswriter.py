...
class GDSDesign:
    ...
    def add_regular_array_escape_four_sided(self, trace_cell_name, layer_name, pitch_x, pitch_y, array_size_x, array_size_y, trace_width, pad_diameter, escape_extent=100, routing_angle=45):
        ...
        for split in right_split:
            ...
            for i in range(len(split)):
                ...
                # Special case for only one trace in a column to route out
                    ...
                    assert 3*trace_width <= effective_pitch_y, "Not enough space for traces."
                    a, b = split[i]
                    if b < int(array_size_y/2):
        ...
        if array_size_y % 2 == 0:
            ...
            if special_split[:, 0].min() not in bottom_route:
            ...
            if special_split[:, 0].min() not in top_route:
        ...
        if array_size_x % 2 == 0:
            ...
            if special_split[:, 1].min() not in left_route:
            ...
            if special_split[:, 1].min() not in right_route:
        ...
        if array_size_y % 2 == 0:
            ...
            if special_split[:, 0].max() not in bottom_route:
            ...
            if special_split[:, 0].max() not in top_route:
